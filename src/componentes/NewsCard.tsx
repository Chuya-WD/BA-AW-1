/**
 * @file NewsCard.tsx
 * @description Componente de tarjeta para mostrar noticias individuales.
 * Soporta dos variantes: estándar y destacada (isFeatured).
 */
...
export default function NewsCard({ news, isFeatured = false }: NewsCardProps) {
  return (
    <div
      className={`group relative overflow-hidden bg-gray-900 shadow-lg transition-all hover:scale-[1.01] ${isFeatured ? "h-full min-h-[400px]" : "h-[190px]"
        }`}
    >
      {/* Imagen de fondo */}
      ...
      <div className="absolute inset-0 left-0 p-6 w-full flex flex-col justify-end">
        <h3
          className={`font-bold leading-tight text-white mb-3 text-left ${isFeatured ? "text-2xl" : "text-sm md:text-base"
            }`}
        >
          {news.title}
        </h3>
