    ...
    <div className="group h-[400px] w-full [perspective:1200px]">
      ...
      <div className="relative h-full w-full rounded-3xl transition-transform duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] shadow-2xl">
        ...
        <div className="absolute inset-0 h-full w-full rounded-3xl bg-gradient-to-b from-white to-gray-50 flex flex-col items-center justify-center p-8 [backface-visibility:hidden] border border-slate-200">
          <div className="relative w-36 h-36 mb-6 ring-4 ring-offset-4 ring-[#0f2c4c] rounded-full overflow-hidden shadow-inner">
            <Image 
              src={image} 
              alt={`Retrato de ${name}`}
              fill
          ...
          <div className="text-center">
            <h3 className="text-2xl font-black text-[#0f2c4c] tracking-tight">{name}</h3>
            <p className="text-sm font-semibold text-blue-600/70 uppercase tracking-widest mt-1">{role}</p>
          </div>

          <div className="absolute bottom-8 flex items-center gap-2 px-4 py-2 bg-[#0f2c4c]/5 rounded-full text-[#0f2c4c] text-[10px] font-bold uppercase">
            <span>Interactuar</span>
            <div className="w-1.5 h-1.5 rounded-full bg-[#0f2c4c] animate-pulse" />
          </div>
        </div>

        {/* --- CARA TRASERA --- */}
        <div className="absolute inset-0 h-full w-full rounded-3xl bg-[#0f2c4c] p-10 text-white [backface-visibility:hidden] [transform:rotateY(180deg)] flex flex-col justify-center items-center text-center shadow-[inset_0_0_50px_rgba(0,0,0,0.2)]">
